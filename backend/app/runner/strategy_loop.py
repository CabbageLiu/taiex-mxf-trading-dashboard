...
class StrategyLoop:
    ...
    async def _fire(self, signal: Signal, channels: list[str]) -> None:
        # Stamp the latest TrendService snapshot onto the signal payload BEFORE
        # we persist so the trend_label/trend_score travel both to signals.payload
        # (visible to /signals and downstream consumers) and to dispatch
        # (Discord, n8n, in-app WS feed).
        if self._trend_service is not None:
            try:
                snap = self._trend_service.latest()
            except Exception:
                snap = None
                log.exception("trend_service.latest() raised in StrategyLoop._fire")
            if snap is not None:
                signal.payload.setdefault("trend_label", snap.label)
                signal.payload.setdefault("trend_score", round(snap.score, 2))
