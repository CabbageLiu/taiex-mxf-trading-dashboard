...
class MA:
    ...
    def compute(self, bars: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        # both (e.g. a close-vs-MA alignment gate on an aux resolution) can
        # read a single aux frame without a second lookup. Additive; existing
        # readers of the ``ma`` column are unaffected.
