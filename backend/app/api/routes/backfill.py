...
FinMind) and `inserted` (rows actually persisted; ON CONFLICT NO-OPs are
excluded). Useful for backtesting setup — pull a wide historical window
once, then run strategies against the resulting `ticks` / continuous
aggregates.

Auth: none in V2 (documented gap). Tailscale-only deploys are fine; do not
expose this surface publicly without first wiring the `alert_secret` header
check.
"""
...
async def post_backfill(
    ...
    if not settings.finmind_token:
        raise HTTPException(503, "FINMIND_TOKEN not configured on server")
    try:
