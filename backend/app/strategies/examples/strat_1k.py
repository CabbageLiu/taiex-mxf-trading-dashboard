...
Single-resolution LONG-only strategy. Entry on 1m bar close when 4 gates
align on a rising-edge transition (false → true). Exit on TP / SL / trailing
stop / DI_JUMP, evaluated on each subsequent 1m bar close.
...
Exit priority (per 1m bar close, first match wins):
  TP          — pnl ≥ 50
  SL          — pnl ≤ −40
  TRAIL       — pnl ≤ peak_pnl − 50 (peak tracked from entry, starts at 0)
  DI_JUMP_1M  — minus_di[-1] − minus_di[-2] > 5 (strict `>`, not `>=`)
...
class TradeStrat1K(Strategy):
    ...
    def on_bar(self, ev: BarEvent) -> Signal | None:
        ...
        params: TradeStrat1KParams = self.params  # type: ignore[assignment]
    ...
    def _manage_position(
        self, ev: BarEvent, st: _StratState, p: TradeStrat1KParams
    ) -> Signal | None:
        pos = st.position
        if pos is None:
            return None
        ...
        # DI_JUMP: -DI just jumped > di_jump_points in one bar.
        if dmi is not None:
            ...
            if (
                ...
                return self._close_position(
                    ev, st, close, "DI_JUMP_1M", pnl, exit_ind=snapshot
                )
    ...
    def _maybe_enter(
        ...
        # Cooldown blocks AND consumes one bar; see strat_30k for rationale.
