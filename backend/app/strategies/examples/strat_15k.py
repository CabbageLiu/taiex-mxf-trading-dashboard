...
Single-resolution LONG-only strategy. Entry on 15m bar close when 4 gates
align on a rising-edge transition (false → true). Exit on TP / SL / trailing
stop, evaluated on each subsequent 15m bar close.
...
Exit priority (per 15m bar close, first match wins):
  TP   — pnl ≥ 130
  SL   — pnl ≤ −70
...
class TradeStrat15K(Strategy):
    ...
    description: ClassVar[str] = (
        "15 分鐘多單策略；進場：close>MA120 且 MA120 向上、KD 連兩 KS>DS、"
        "MACD 直方翻正、+DI>-DI 且 -DI 縮；出場：TP 130 / SL −70 / 移動停損 80。"
    )
    ...
    def on_bar(self, ev: BarEvent) -> Signal | None:
        ...
        params: TradeStrat15KParams = self.params  # type: ignore[assignment]
    ...
    def _manage_position(
        self, ev: BarEvent, st: _StratState, p: TradeStrat15KParams
    ) -> Signal | None:
        pos = st.position
        if pos is None:
            return None
