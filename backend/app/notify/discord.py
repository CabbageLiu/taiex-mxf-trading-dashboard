...
def _entry_condition_summary_tc(strategy_name: str) -> str:
    ...
    dominance; strat_30k/15k share MA120 trend + KD + MACD + DMI flip;
    strat_1k drops the MA120 gate and is AND-only on KD/MACD/DMI). For
    unknown strategies returns a generic phrase. Indicator names stay
    English.
    """
    if strategy_name in {"trade_strat_v1", "trade_strat_v2"}:
        return "KD > 20 / MACD 翻正 / +DI > 21 且 +DI > -DI"
    if strategy_name in {"strat_30k", "strat_15k"}:
        return "K 棒在 MA120 之上且 MA120 向上 / KD 連兩 KS>DS / MACD 翻正 / +DI>-DI 且 -DI 縮"
    if strategy_name == "strat_1k":
        return "1m +DI 上升 -DI 下降 / KD 連兩 KS>DS 且首根 KS<70 / MACD 直方>0"
    if strategy_name == "strat_1k_ai":
        return (
            "1m +DI 上升 -DI 下降 / KD 連兩 KS>DS 且首根 KS<70 / MACD 直方>0 "
            "(AI 出場：硬停損 / 保本 / 移動停損 / 崩盤過濾 / 時間截止)"
        )
    return "進場條件達標"
