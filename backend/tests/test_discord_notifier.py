...
class _DummyStrat(Strategy):
    """Self-contained strategy so the display_name / entry-summary lookups
    don't depend on any particular example module being present."""

    name = "dummy_strat"
    display_name = "測試策略"
    entry_summary_tc = "KD 連兩 K>D / MACD 翻正 / +DI > -DI"
...
def _register_dummy_strategy() -> Any:
    """Register the dummy strategy for this module only; remove on teardown
    so other test modules see an unpolluted registry."""
    _registry[_DummyStrat.name] = _DummyStrat
    yield
    _registry.pop(_DummyStrat.name, None)
...
async def test_discord_embed_open_description_lists_entry_conditions():
    n = DiscordNotifier(url="https://discord.example/webhook")
    sig = _make_signal(side="LONG")  # dummy_strat
    with patch("app.notify.discord.httpx.AsyncClient", _StubClient):
        await n.send(sig)
    desc = _StubClient.last_body["embeds"][0]["description"]
