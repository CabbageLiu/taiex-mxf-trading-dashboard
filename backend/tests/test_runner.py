...
# ---------------------------------------------------------------------------
# Feed-health watchdog (tick-starvation → forced reconnect)
# ---------------------------------------------------------------------------


class _ReconnectAdapter(_SilentAdapter):
    """Silent adapter that also exposes an awaitable ``reconnect`` spy."""

    def __init__(self) -> None:
        self.reconnect = AsyncMock()


def _stale_runner(adapter):
    """Runner primed so the feed reads stale + in-session, watchdog enabled."""
    runner = IngestRunner(adapter=adapter)
    tz = runner._settings.tz
    runner._feed_started_at = datetime.now(tz) - timedelta(seconds=300)
    runner._last_tick = None
    runner._in_market_session = lambda now: True  # type: ignore[assignment]
    return runner


@pytest.mark.asyncio
async def test_feed_health_forces_reconnect_when_stale_in_session():
    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    await runner._check_feed_health()
    adapter.reconnect.assert_awaited_once()
    assert runner._reconnect_count == 1
    assert runner._session_reconnects == 1
    assert runner._reconnect_failures == 1  # stays armed until ticks resume


@pytest.mark.asyncio
async def test_feed_health_noop_off_session():
    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    runner._in_market_session = lambda now: False  # type: ignore[assignment]
    await runner._check_feed_health()
    adapter.reconnect.assert_not_awaited()


@pytest.mark.asyncio
async def test_feed_health_noop_when_ticks_recent():
    from app.adapters.base import Tick

    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    tz = runner._settings.tz
    runner._last_tick = Tick(ts=datetime.now(tz), symbol="MXF", price=39000.0, source="T")
    await runner._check_feed_health()
    adapter.reconnect.assert_not_awaited()


@pytest.mark.asyncio
async def test_feed_health_respects_backoff_window():
    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    await runner._check_feed_health()      # first reconnect fires
    await runner._check_feed_health()      # immediately again → inside backoff
    adapter.reconnect.assert_awaited_once()


@pytest.mark.asyncio
async def test_feed_health_resets_when_ticks_resume():
    from app.adapters.base import Tick

    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    await runner._check_feed_health()
    assert runner._reconnect_failures == 1
    # Ticks resume → next pass resets backoff + session counter.
    runner._last_tick = Tick(
        ts=datetime.now(runner._settings.tz), symbol="MXF", price=39000.0, source="T"
    )
    await runner._check_feed_health()
    assert runner._reconnect_failures == 0
    assert runner._session_reconnects == 0


@pytest.mark.asyncio
async def test_feed_health_hard_session_cap():
    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    cap = runner._settings.feed_max_reconnects_per_session
    # Drive past the cap, neutralising the backoff gate each pass.
    for _ in range(cap + 3):
        runner._last_reconnect_at = None  # bypass backoff for the test
        await runner._check_feed_health()
    assert adapter.reconnect.await_count == cap


@pytest.mark.asyncio
async def test_feed_health_adapter_without_reconnect_is_safe():
    runner = _stale_runner(_SilentAdapter())  # no reconnect attr
    await runner._check_feed_health()  # must not raise
    assert runner._reconnect_count == 0


@pytest.mark.asyncio
async def test_feed_health_disabled_by_flag(monkeypatch):
    adapter = _ReconnectAdapter()
    runner = _stale_runner(adapter)
    monkeypatch.setattr(runner._settings, "feed_watchdog_enabled", False, raising=False)
    await runner._check_feed_health()
    adapter.reconnect.assert_not_awaited()


@pytest.mark.asyncio
async def test_feed_health_dict_shape():
    runner = _stale_runner(_ReconnectAdapter())
    fh = runner.feed_health()
    assert fh["in_market_session"] is True
    assert fh["feed_healthy"] is False  # stale + in session
    assert set(fh) >= {
        "feed_healthy",
        "in_market_session",
        "silence_seconds",
