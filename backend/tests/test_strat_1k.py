...
def test_aux_indicator_specs_empty():
    """Spec drops the 2m aux gates entirely."""
    assert TradeStrat1K.aux_indicator_specs == {}
...
def test_trail_fires_on_immediate_drawdown_no_sl():
    ...
    bars = _bars(5, last_close=100.0)  # pnl = -100, trail at -40
    inds = _inds(bars)
    sig = TradeStrat1K(params=TradeStrat1KParams()).on_bar(_event(bars, inds))
    assert sig is not None
    assert sig.side == "EXIT"
    assert sig.payload["exit_reason"] == "TRAIL"
    # pnl_points reflects the actual drawdown at evaluation, not the trail threshold.
    assert sig.payload["pnl_points"] == -100.0
...
def test_exit_runs_outside_window():
    """Open at 13:00 Taipei (in window); tick at 14:30 (closed gap) with
    +40 → EXIT TP fires (window gate is entry-only)."""
    strat = TradeStrat1K(params=TradeStrat1KParams())
